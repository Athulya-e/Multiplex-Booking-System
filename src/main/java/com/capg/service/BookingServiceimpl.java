...
public class BookingServiceimpl implements BookingService {
	...
	public Booking createNewBooking(BookingDto bookingDto) {
		...
		SeatType seatType = seatTypeRepository.findById(bookingDto.getSeatTypeId()).get();
		if(seatType == null) {
		...
		if (bookingDto.getShowDate().isBefore(LocalDate.now())||bookingDto.getShowDate().isBefore(bookingDto.getBookingDate())) {
			...
			ShowAvailability showAvailable =	showAvailRepository.findBySeatTypeAndAvailableDateAndShow(seatType, bookingDto.getShowDate(),show1);
...
System.out.println("--------------"+showAvailable);
			...
			bookingNew.setSeatDescription(seatType.getSeatTypeDescription());
			bookingNew.setSeatFare(seatType.getSeatFare());
			bookingNew.setNoOfSeats(bookingDto.getNoOfSeats());
			bookingNew.setBookingAmount(bookingDto.getNoOfSeats() * seatType.getSeatFare());
